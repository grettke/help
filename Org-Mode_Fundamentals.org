...
#+BEGIN_SRC emacs-lisp :results output silent :comments nos
;; -*- lexical-binding: t -*-
#+END_SRC
...
- After thoughtless changing this to =no= I found that it worked just fine
  - Less space between the lines makes the generated code a little bit easier
    for guests to swallow
  - Can always add spaces when desired in key locations
