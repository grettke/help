...
- Steps
  - Review and Source =.Rsetup= prepare the OS for R and R for your configuration
  ...
  - Review and Source =Rinstall= to install required packages
    - When R starts, it won't find any of the required packages. It will
      ...
      it is happy: =source("~/.Rinstall")=. Then restart R and everything should
      be fine.
...
#+begin_src sh :tangle .Rsetup.sh :comments no
«rsetup-defs»
#+end_src
